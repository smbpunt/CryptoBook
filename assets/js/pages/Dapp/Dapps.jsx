...
import API from "../../services/dappAPI";
    ...
    return (
        <div className="container">
            <div className="d-flex justify-content-between align-items-center mb-3">
                <h1>Liste des dapps</h1>
                <Link to="/dapps/new" className="btn btn-primary">Creer</Link>
            </div>
                    ...
                    paginated.map(
                        dapp => (
                            <tr key={dapp.id}>
                                <td>{dapp.libelle}</td>
                                <td>{dapp.url}</td>
                                <td>{dapp.blockchain.libelle}</td>
                                <td className="text-end">
                                    <div className="btn-group" role="group">
                                        <Link to={"/dapps/" + dapp.id} className="btn btn-primary btn-sm">Modifier</Link>
                                        <button onClick={() => handleDelete(dapp.id)}
